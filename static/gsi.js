...
(()=>{
 ...
 const [dataResult,figuresResult]=results;gallery=figuresResult.status==='fulfilled'?figuresResult.value.map(f=>({...f,file:f.file||f.png})):[];chartOptions();chart();
 ...
 if(loaded.environment!==config.label)throw Error('O índice não corresponde ao ambiente '+config.label+'. Confira config.js e a geração dos dados.');
